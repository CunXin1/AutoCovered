...
3. 收益核算(用 metrics 与 alerts 流水中的数字,不要自行估算):
   - 本周/本月权利金浮盈变化(各仓 option_pnl_per_share × 张数 × 100)
   - **诚实指标**:被 call 封顶而放弃的上涨(BREACHED 仓的 现价−strike 部分)
     vs 收到的权利金 — covered call 在暴涨市会跑输,直说
...
6. 运维检查:positions.json 的 updated_at 与 data_source 是否正常;
   `state/proposals.json` 有无遗留 pending;如启用了 Schwab 直连,提醒
